---
name: neat-freak
description: 任务完成后同步项目文档、AGENTS.md、CLAUDE.md 与记忆，输出变更摘要。用户说整理一下、同步一下或 neat 时使用。
---
...
> **Agent 记忆系统的具体位置因平台而异**（Codex 在 `~/.Codex/projects/<...>/memory/`，Codex 用 `AGENTS.md`，OpenCode 用 `.opencode/`，OpenClaw 用 `~/.openclaw/`）。完整路径速查见 references/agent-paths.md。如果当前 agent 没有独立的记忆系统，直接跳过这一层，把功夫全花在 docs 和项目根 markdown 上。
...
*   **references/agent-paths.md** — Codex / Codex / OpenCode 各自的记忆与配置路径速查
