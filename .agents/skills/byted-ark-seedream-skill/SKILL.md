...
这个 skill 不是只有说明书，真实可执行脚本就在本 skill 文件夹内：
...
```bash
cd "/Users/kunki/ObsidianVaults/领航知识库/.claude/skills/byted-ark-seedream-skill"
ARK_SEEDREAM_SAVE_PATH="/Users/kunki/ObsidianVaults/领航知识库/06-归档与交付/outputs/<项目名>/assets" \
...
脚本已支持自动读取 Agent Plan 专用配置：

```text
~/.config/agentplan/ark_api_key
~/.config/ark/api_key
环境变量 AGENTPLAN_ARK_API_KEY / ARK_API_KEY / ARK_API_KEY_AGENTPLAN
```
...
所以后续不要再向用户索要 key，除非脚本明确报 key 不存在或失效。

### 保存路径规则

- 所有最终图片必须保存到 workspace 内，优先放到当前项目 `assets/` 目录。
- 脚本默认会在保存目录下再建日期文件夹，例如 `assets/2026-06-20/seedream_xxx.png`。
- 如果用户要固定文件名，生成后复制/重命名到目标路径。

### 图片预览规则（重要踩坑）

回复中展示本地图片时，**不要使用 `file://` 前缀**，否则 NewMax/聊天 UI 可能无法预览。

✅ 正确：

```markdown
![描述](/Users/kunki/ObsidianVaults/领航知识库/06-归档与交付/outputs/xxx.png)
```

❌ 错误：

```markdown
![描述](file:///Users/kunki/ObsidianVaults/领航知识库/06-归档与交付/outputs/xxx.png)
```

### 常见出图问题

- 证书、背景板、海报上的文字容易乱码：需要提示词明确“文字模糊不可读”或“证书只写四个清晰汉字：获奖证书”。
- `size` 建议用 `1K/2K/3K/4K`，不要乱传非支持尺寸。16:9 需求写进 prompt：`16:9 horizontal composition`。
