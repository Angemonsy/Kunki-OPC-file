...
function autoDetectApiKey() {
  // 优先级 0: Agent Plan 专用配置（推荐，不污染 Claude/Hermes/OpenClaw 模型配置）
  const dedicatedEnvNames = ['AGENTPLAN_ARK_API_KEY', 'ARK_API_KEY', 'ARK_API_KEY_AGENTPLAN'];
  for (const envName of dedicatedEnvNames) {
    const value = process.env[envName];
    if (typeof value === 'string') {
      const validation = validateArkKey(value);
      if (validation.valid) {
        return { key: validation.trimmed, source: envName, found: true };
      }
    }
  }

  const dedicatedKeyFiles = [
    path.join(homedir, '.config', 'agentplan', 'ark_api_key'),
    path.join(homedir, '.config', 'ark', 'api_key')
  ];
  for (const keyPath of dedicatedKeyFiles) {
    if (fs.existsSync(keyPath)) {
      try {
        const raw = fs.readFileSync(keyPath, 'utf8').trim();
        const validation = validateArkKey(raw);
        if (validation.valid) {
          return { key: validation.trimmed, source: keyPath, found: true };
        }
      } catch (e) {
        // 专用配置读取失败，继续尝试其他位置
      }
