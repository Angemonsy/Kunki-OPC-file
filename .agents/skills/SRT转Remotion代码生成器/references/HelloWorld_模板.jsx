...
export const HelloWorld = () => {
  ...
  // 【变量7】topTitle：顶部大标题（竖屏专用）
  // 格式：两行文字数组，每个元素有 text, bold(可选), size(可选)
  // const topTitle = [
  //   [{text: "关键词", bold: true, size: 96}, {text: "越火", size: 88}],
  //   [{text: "知识库", bold: true, size: 96}, {text: "越值钱", size: 88}]
  // ];
  const topTitle = null;
  ...
  return (
    <div style={{
      ...
      {/* 顶部大标题 */}
      {topTitle && (() => {
        const revealPos = interpolate(frame, [0, fps * 3], [-20, 120], {extrapolateRight: 'clamp'});
        return (
      <div style={{
        position: 'absolute',
        top: 160,
        ...
        padding: '28px 48px',
        WebkitMaskImage: `linear-gradient(to right, black ${revealPos - 20}%, transparent ${revealPos}%)`,
        maskImage: `linear-gradient(to right, black ${revealPos - 20}%, transparent ${revealPos}%)`,
      }}>
        {topTitle.map((titleLine, li) => (
          <div key={li} style={{
            fontFamily: `${fontFamily}, -apple-system, SF Pro Display, system-ui, sans-serif`,
            fontSize: 88,
            fontWeight: '300',
            ...
            {titleLine.map((seg, si) => (
              <span key={si} style={{
                fontWeight: seg.bold ? '900' : '300',
                color: seg.bold ? '#ffffff' : 'rgba(255,255,255,0.85)',
                fontSize: seg.size || 88,
              }}>{seg.text}</span>
            ))}
          </div>
        ))}
      ...
      {videos.map((v, vi) => (
        currentTime >= v.start && currentTime < v.end && (
          <div key={vi} style={{
            ...
            left: 0,
            width: '100%',
            height: topTitle ? 'calc(100% - 420px)' : '100%',
            zIndex: 5,
            pointerEvents: 'none',
          }}>
            <Video
      ...
      {/* PPT 关键词区域（居中，限制在标题以下） */}
      <div style={{
        position: 'absolute',
        top: topTitle ? 420 : 0,
        left: 0,
        width: '100%',
        height: topTitle ? `calc(100% - 420px - 200px)` : `calc(100% - 200px)`,
        display: 'flex',
        flexDirection: 'column',
        gap: 24,
        alignItems: 'center',
