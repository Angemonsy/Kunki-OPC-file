...
当API返回 `"success": true` 后：

1. 判断源文件是否在 `02.领域/0.内容创作系统/🔥02.发布文案/2026年/待发布/公众号/` 目录下
2. 如果是 → 自动将文件移动到 `02.领域/0.内容创作系统/🔥02.发布文案/2026年/已发布/公众号/` 目录
3. 更新文件frontmatter中的 `status` 字段：`待发布` → `已发布`
4. 添加 `published_date` 字段记录实际发布时间
5. 告知用户："文章已发布到草稿箱，文件已从待发布移到已发布"

**执行命令示例：**
```bash
# 移动文件
mv "02.领域/0.内容创作系统/🔥02.发布文案/2026年/待发布/公众号/文章.md" "02.领域/0.内容创作系统/🔥02.发布文案/2026年/已发布/公众号/文章.md"
```

**注意事项：**
- 如果已发布目录不存在，自动创建
- 如果源文件不在待发布目录（比如用户直接指定了其他路径），跳过移动步骤
- 移动失败不影响发布结果，但要告知用户手动移动
