...
- Accounts configured in `.agents/context/tiktok-accounts.md`
...
```bash
python3 .agents/skills/TikTok研究分析/scripts/fetch_tiktok.py \
  --days 30 \
  --limit 50 \
  --sorting latest \
...
```bash
python3 .agents/skills/TikTok研究分析/scripts/analyze_posts.py \
...
```bash
python3 .agents/skills/video-content-analyzer/scripts/analyze_videos.py \
...
python3 .agents/skills/TikTok研究分析/scripts/fetch_tiktok.py -o "$RUN_FOLDER/raw.json" && \
python3 .agents/skills/TikTok研究分析/scripts/analyze_posts.py -i "$RUN_FOLDER/raw.json" -o "$RUN_FOLDER/outliers.json" && \
python3 .agents/skills/video-content-analyzer/scripts/analyze_videos.py -i "$RUN_FOLDER/outliers.json" -o "$RUN_FOLDER/video-analysis.json" -p tiktok
```
